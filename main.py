...
dp = Dispatcher(storage=None)  # MemoryStorage встроено в FSMContext
...
async def create_jira_ticket(
    ...
    async with aiohttp.ClientSession(auth=auth) as session:
        ...
        # Уведомление красиво
        notify_text = (
            f"📨 <b>Новая подзадача создана!</b>\n"
            f"🔑 <b>{issue_key}</b>\n"
            f"👤 Автор: <b>{author}</b>\n"
            f"📝 Описание:\n{text}\n\n"
            f"🔗 <a href=\"{JIRA_URL}/browse/{issue_key}\">Перейти к задаче</a>"
        )
